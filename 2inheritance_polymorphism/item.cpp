...
bool operator <(Item& i1, Item& i2){return i1.bill()<i2.bill();}
//will di < i apply the discount to di?
...
int main()
{
  ...
  DiscountItem di = DiscountItem(11.00, 10);
  //One item at $11.00 with a 10% discount=$9.90
  cout << di.bill() << endl;
  
  //What  happens?
  Item i2 = DiscountItem(11.00, 10);
  cout<<"i2.bill()=" <<i2.bill()<<endl;//Doesn't call DiscountItem's bill() - we need to use a pointer!
  //bill is the function we want to do polymorphism/dynamic dispatch with

  //What happens?
  Item* ip2 = new DiscountItem(11.00, 10); 
  cout << "ip2->bill()=" << ip2->bill()<<endl;//This one will call DiscountItem's bill()


  if(di<i){//i $10; di $11 10%disc=$9.90, 
    cout<<"Discounted!"<<endl;//Because parameters to < are cbr this will call DiscountItem's bill() for di
  }
  else{
